...
def scrapbook(request):
    notes = Note.objects.filter(user1=request.user).union(Note.objects.filter(user2=request.user)).union(Note.objects.filter(last_editor=request.user)).order_by('-last_updated')  # Use '-last_updated' to order by last updated date
